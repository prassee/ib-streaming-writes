"""Stream `Screen View` events from `staging.user_activity` into `core.screen_view`.

- Creates the `core` namespace and `core.screen_view` (partitioned by event day)
  in the default catalog (spark.sql.defaultCatalog) if they don't exist.
- Streams every append snapshot on the source, keeps rows where t = 'Screen View'
  and appends them with the columns that apply to screen views. Events are
  immutable, so this is a plain append; Iceberg's streaming sink commits each
  micro-batch once, so a replayed batch after a restart isn't written twice.
- On first start it processes the whole history, then picks up new appends as
  they land.

    just submit stream_screen_view.py                  # run continuously
    just submit stream_screen_view.py --available-now  # process what's there, then exit
"""
...
def screen_views(events: DataFrame) -> DataFrame:
...
def create_target_table(spark: SparkSession, catalog: str, source: str, target: str) -> None:
    ...
    screen_views(spark.table(source).limit(0)).createOrReplaceTempView("screen_view_schema")
    spark.sql(
        ...
        f"AS SELECT * FROM screen_view_schema"
    )
...
def main() -> None:
    ...
    spark = SparkSession.builder.appName("stream-screen-view").getOrCreate()
    catalog = (
